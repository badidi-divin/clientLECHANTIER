	...
	if (isset($_POST['envoie'])) {
		...
		if (!empty($username) && !empty($password1) && !empty($password2)) {
			if($password1<>$password2){
				...
				$requiser=$pdo->prepare("INSERT INTO user SET username=?,password=?,role=?,extension=?");
				$requiser->execute(array($username,$password,$role,$extension));
	...
	if (isset($_POST['envoie2'])) {
		...
		if (!empty($username) && !empty($password)) {
			...
			if ($userexist==1) {
				...
				$_SESSION['extension']=$userinfo['extension'];
	...
	if (isset($_GET['dk'])) {
		...
		if (isset($_POST['envoie7'])) {
			...
			if (empty($_POST['password'])) {
				$ps=$pdo->prepare("UPDATE user SET username=?,role=?,extension=? WHERE id=?");
				//Pour bien recupere les données on crée un table de parametre
				$params=array($username,$role,$extension,$_GET['dk']);
				//Execution de la requête par leur paramètre en ordre 
				$ps->execute($params);
				$success['cool']="Opération reussie";
			}else{
				$password=md5($_POST['password']);
				$ps=$pdo->prepare("UPDATE user SET username=?,password=?,role=?,extension=? WHERE id=?");
				//Pour bien recupere les données on crée un table de parametre
				$params=array($username,$password,$role,$extension,$_GET['dk']);
				//Execution de la requête par leur paramètre en ordre 
				$ps->execute($params);
				$success['cool']="Opération reussie";	
			}
